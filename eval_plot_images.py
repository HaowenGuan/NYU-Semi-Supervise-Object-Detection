...
# Selena: "/data/labeled"
# Caesar: "/data/sbcaesar/nyu/labeled_data_yml/"
...
def evaluate(model, data_loader, device):
    ...
    for images, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        fig, ax = plt.subplots()
        img = plt.imread("/data/sbcaesar/nyu/labeled_data_yml/validation/images/" + str(id) + ".JPEG")
        ax.imshow(img)
        colors = iter(plt.cm.rainbow(np.linspace(0, 1, outputs[0]['labels'].shape[0])))
        for i in range(outputs[0]['labels'].shape[0]):
            if outputs[0]['scores'][i] > 0.5:
                x1, y1, x2, y2 = outputs[0]['boxes'][i]
                name = num_dict[int(outputs[0]['labels'][i])]
                rect = patches.Rectangle((x1, y1), x2 - x1, y2 - y1, linewidth=1, edgecolor=next(colors), facecolor='none', label=name)
                ax.add_patch(rect)
        ax.set_title("Image" + str(id) + " Prediction")
        ax.legend()
        fig.savefig('./output/images/'+ str(id) +'_prediction.JPEG', dpi=100)
